...
def select_pizzas(menu, extras, max_pizzas=5):
    ...
    while total_pizzas < max_pizzas:
        ...
        if 1 <= pizza_choice <= len(menu):
            ...
            topping_count = 0
            while True:
                if topping_count >= MAX_TOPPINGS_PER_PIZZA:
                    print(f"Maximum toppings ({MAX_TOPPINGS_PER_PIZZA}) reached for this pizza.")
                    break

                # Display the topping menu with numbers and the maximum allowed toppings
                print(f"Toppings (Max {MAX_TOPPINGS_PER_PIZZA}):")
